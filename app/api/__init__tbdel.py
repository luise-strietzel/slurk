...
def get_layouts():
    ...
    return jsonify(
        [
            dict(uri="/layout/" + str(layout.id), **layout.as_dict())
            for layout in Layout.query.all()
        ]
    )
...
def get_tokens():
    ...
    return jsonify(
        [
            dict(uri="/token/" + str(token.id), **token.as_dict())
...
def get_users():
    return jsonify(
        [
            dict(uri="/users/" + str(user.id), **user.as_dict())
...
def put_user(id):
    ...
    if "name" in data and isinstance(data["name"], str):
        ...
        return make_response(
            jsonify({"error": "No valid modifiable user attribute found."}), 400
        )
    try:
        db.session.commit()
        return jsonify(user.as_dict())
...
def get_tasks():
    ...
    return jsonify(
        [
            dict(uri="/task/" + str(task.id), **task.as_dict())
    ...
    try:
        if "num_users" in data:
            try:
                ...
                return make_response(
                    jsonify({"error": "invalid number: `num_users`"}, 400)
                )
        if "name" in data:
            task.name = data["name"]
        if "layout" in data and data["layout"]:
...
def get_rooms():
    ...
    return jsonify(
        [dict(uri="/room/" + room.name, **room.as_dict()) for room in Room.query.all()]
    )
...
def get_room(name):
    ...
    if (
        room not in g.current_user.current_rooms
        and not g.current_permissions.room_query
    ):
        return make_response(jsonify({"error": "insufficient rights"}), 403)
...
def get_room_layout(name):
    room = Room.query.get(name)
    if room not in g.current_user.current_rooms and (
        ...
        return jsonify(
            {
                room.name: list(
                    filter_private_messages(
                        [log.as_dict() for log in room.logs], user.id
                    )
                )
